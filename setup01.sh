...
mkdir -p /etc/xray
# JANGAN touch /etc/xray/domain di sini — biar tidak timpa domain yang sudah ada
...
# ==========================================
# Pre-check: IP (satu variabel, konsisten)
# ==========================================
...
function pasang_domain() {
    ...
    if [[ $host == "1" ]]; then
        ...
        echo -e "${green}Domain tersimpan: $host1${NC}"
        sleep 2
        clear
...
function make_folder_xray() {
    ...
    # JANGAN TIMPA DOMAIN yang sudah diset di pasang_domain
...
# ==========================================
# FUNCTION: SSH Setup
# NOTE: rc.local TIDAK ditulis di sini,
#       ditulis sekali di function profile()
# ==========================================
function ssh_setup() {
    clear
    ...
    # Setup rc-local service saja, isi rc.local ditulis sekali di profile()
...
function ins_openvpn() {
    ...
    wget -q "${REPO}files/openvpn" -O /tmp/openvpn
    chmod +x /tmp/openvpn
    bash /tmp/openvpn
...
# NOTE: rc.local HANYA ditulis di sini (satu tempat)
# ==========================================
function profile() {
    clear
...
END
    ...
    # =============================================
    # rc.local DITULIS SEKALI DI SINI (tidak di ssh_setup)
    # Gabungan: disable IPv6 + iptables rules
    # =============================================
...
# NOTE: Menggabungkan tugas ins_restart,
#       tidak perlu ins_restart lagi
# ==========================================
function enable_services() {
    ...
    systemctl enable xray nginx haproxy cron ssh dropbear vnstat 2>/dev/null || true
    systemctl enable openvpn-server@server-tcp openvpn-server@server-udp 2>/dev/null || true
...
# Urutan yang benar:
# 1. pasang_domain      → domain tersimpan
# 2. make_folder_xray   → folder siap, domain tidak ditimpa
# 3. base_package       → semua dependency terinstall
# 4. first_setup        → haproxy terinstall
# 5. nginx_install      → nginx terinstall
# 6. pasang_ssl         → SSL siap
# 7. install_xray       → xray + haproxy + nginx dikonfigurasi
# 8. dst...
# ==========================================
function instal() {
    clear
    ...
    ins_epro
    menu
    profile          # rc.local ditulis di sini
    enable_services  # restart semua service (menggantikan ins_restart)
    restart_system
}
